...
export default function ListingForm({ userRole, token, backUrl, successUrl, warehouses = [] }: ListingFormProps) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/commodity-types`);
  ...
  const removeImage = async (publicId: string) => {
    ...
    try {
      await fetch(`${API_URL}/upload/${publicId}`, {
        method: 'DELETE',
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {loadingCategories ? (
                <div className="flex items-center gap-2 text-sm text-gray-400 py-3">
                  <div className="w-4 h-4 border-2 border-gray-300 border-t-elba-secondary rounded-full animate-spin" />
                  Loading categories...
                </div>
              ) : categories.length === 0 ? (
                <div className="text-sm text-gray-400 py-3 bg-yellow-50 rounded-xl px-4">
                  No categories found. Please check if the backend is running.
                  <button 
                    type="button" 
                    onClick={() => window.location.reload()}
                    className="ml-2 text-elba-secondary hover:underline"
                  >
                    Refresh
                  </button>
                </div>
        ...
        {/* Rest of form - only show if name and category are filled */}
        {formData.commodityName.trim() && formData.category && (
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6">
              <h2 className="font-semibold text-elba-primary mb-1 flex items-center gap-2">
                <ImagePlus className="w-5 h-5 text-elba-secondary" />
                Product Images
              </h2>
              ...
              <div className="grid grid-cols-4 gap-3">
                {images.map((img, index) => (
                  <div key={img.publicId} className="relative aspect-square rounded-xl overflow-hidden border border-gray-200 group">
                    ...
                    {index === 0 && (
                      <span className="absolute bottom-1 left-1 text-[8px] bg-elba-primary text-white px-1.5 py-0.5 rounded-full font-bold">
                        Cover
                      </span>
                    )}
                    <button
                ...
                {images.length < MAX_IMAGES && (
                  <button
                    ...
                    disabled={uploading}
                    className="aspect-square rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 hover:border-elba-secondary hover:text-elba-secondary transition-colors disabled:opacity-50"
                  >
                    {uploading ? (
            ...
            {/* Submit */}
            <button type="submit" disabled={submitting || !isFormValid || uploading}
              ...
              {submitting ? (
                ...
                <>{isAutoApproved ? 'Create Listing' : 'Submit for Review'} <ArrowRight className="w-5 h-5" /></>
              )}
            </button>
          </>
        )}
...
// // client/components/listings/ListingForm.tsx
// 'use client';
...
//         {/* Rest of the form - only show if name and category are filled */}
//         {formData.commodityName.trim() && formData.category && (
//           <>
//             {/* Quantity */}
//             <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6">
