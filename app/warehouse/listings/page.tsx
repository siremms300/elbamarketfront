// client/app/warehouse/listings/page.tsx
'use client';

import { useState, useEffect } from 'react';
import { useAuth } from '@/context/AuthContext';
import Link from 'next/link';
import { Plus, Package, Eye, Trash2, X, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function WarehouseListingsPage() {
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      console.log('Fetching listings with token:', token ? 'Token exists' : 'No token');
  ...
  if (!token) {
    return (
      <div className="text-center py-12 text-gray-400">
        ...
        <p className="text-gray-500 font-medium">Please log in to view your listings.</p>
        <Link href="/login" className="text-elba-secondary text-sm mt-2 inline-block">
          Go to Login
        </Link>
      </div>
    );
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {listing.liveCommodityId && (
                    <Link
                      ...
                      target="_blank"
                      className="text-xs text-elba-secondary font-medium hover:text-elba-secondary-light flex items-center gap-1"
                    >
